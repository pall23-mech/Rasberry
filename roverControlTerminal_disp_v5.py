#!/usr/bin/env python3
# oled_two_buttons_with_leds_17_23_22_24.py
# SSD1306 OLED + Buttons on 17/23 + LEDs on 22/24 (BCM)
...
PIN_LED_MODE   = 22   # MODE LED
PIN_LED_ESTOP  = 24   # ESTOP LED

# Allow/disallow MODE LED during ESTOP
...
# LEDs with clear error if a pin is busy
try:
    led_mode  = LED(PIN_LED_MODE,  pin_factory=factory)
    led_estop = LED(PIN_LED_ESTOP, pin_factory=factory)
except Exception as e:
    log_line(f"GPIO init failed for LEDs {PIN_LED_MODE}/{PIN_LED_ESTOP}: {e}")
    raise
...
show_mode(state.name.replace("_", " "))
log_line(f"Buttons MODE={PIN_BTN_MODE}, ESTOP={PIN_BTN_ESTOP}; LEDs MODE={PIN_LED_MODE}, ESTOP={PIN_LED_ESTOP}")
...
try:
    while running:
        ...
        # Optional guard: keep MODE LED off during ESTOP when suppression enabled
